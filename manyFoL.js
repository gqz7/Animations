...
const utils = {
   ...
   //ANIMATION CYCLE START
    ...
    function render() {
        ...
        const framesMin = -100
        count = 0
        if (growing && frames <= framesMax) {
          
          frames+=speed
        } else if (growing && frames > framesMax) {
          growing = false
        } else if (!growing && frames >= framesMin) {
          frames-=speed
        } else if (!growing && frames < framesMin) {
          growing = true
        }
        renderAll()
        const trans = Math.abs(frames)*3.18
        context.save()
        context.translate(-trans*2,0)
        renderAll()
        context.translate(trans*4,0)
        renderAll()
        context.translate(-trans*2,trans*2)
        renderAll()
        context.translate(0, -trans*4)
        renderAll()
        context.restore()
...
function renderImage() {
    ...
    context.save()
        ...
        context.strokeStyle = `hsl(${111+frames*10+count*5}, 100%, 70%)`
        context.beginPath()
        context.arc(0, 0, 100/gscale, 0, pi*2)
        context.stroke()
