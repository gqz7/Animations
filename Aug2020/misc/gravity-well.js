...
function createImg(s) { 
    ...
    context.save();
        ...
        for (let x = -10; x < width/20+10; x++) {
            
            for (let y = -10; y < height/20+10; y++) {

                const
                noiseX = x/30 + s/50, 
                noiseY = y/30,
                distance = sqrt( pow((x*20)-(width/2), 2) +  pow((y*20)-(height/2), 2) ),
                N1 = Noise(noiseX, noiseY)*distance/100,
                N2 = Noise(noiseY,noiseX)*distance/100,
                radius = 2//+Math.abs(2*N1);
                X = x*20+ N1*20+N2*20,
                Y = y*20+ N1*20-N2*20;

                context.fillStyle = `hsl(${0}, 0%, ${distance/5}%)`;
